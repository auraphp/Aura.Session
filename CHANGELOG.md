...
- (CHG) `PhpredisClient::del()` no longer checks for `UNLINK` support before using it, so `RedisSessionHandler` with that adapter now requires a Redis 4.0 or later server. The `DEL` fallback was untestable dead weight: `Redis::unlink()` has existed since phpredis 4.0.0, and every phpredis build that supports the PHP 8.4 this package requires is far newer than that. `PredisClient::del()` still issues `DEL`, because predis is pure PHP and only registered the `UNLINK` command in 3.5.0, while the adapter supports predis `^2.0 || ^3.0`.
